...
// Convert any value to a plain JSON-safe object (no Date objects)
...
async function getDashboardData() {
    ...
    await Promise.all([
      sql`
        ...
        FROM receipts WHERE parsed = true
          AND week_saturday = (
            SELECT week_saturday FROM receipts WHERE parsed = true ORDER BY receipt_date DESC LIMIT 1
          )
      ...
      sql`
        ...
        FROM receipts WHERE parsed = true
          AND year = EXTRACT(YEAR FROM NOW()) AND month = EXTRACT(MONTH FROM NOW())
        GROUP BY year, month
      `,
      sql`
      ...
      sql`
        SELECT ri.category,
          SUM(ri.total_price) AS total, COUNT(*) AS item_count
        FROM receipt_items ri JOIN receipts r ON ri.receipt_id = r.id
        WHERE r.parsed = true
          AND r.year = EXTRACT(YEAR FROM NOW()) AND r.month = EXTRACT(MONTH FROM NOW())
      ...
      `,
      sql`
        SELECT ri.clean_name, ri.category,
          MIN(CASE WHEN r.receipt_date = fd.min_date THEN ri.unit_price END) AS first_price,
          MIN(CASE WHEN r.receipt_date = ld.max_date THEN ri.unit_price END) AS latest_price,
          COUNT(DISTINCT ri.receipt_id) AS purchase_count
        FROM receipt_items ri JOIN receipts r ON ri.receipt_id = r.id
        JOIN (SELECT clean_name, MIN(r2.receipt_date) AS min_date FROM receipt_items ri2
              JOIN receipts r2 ON ri2.receipt_id = r2.id
              WHERE ri2.clean_name IS NOT NULL AND ri2.unit_price IS NOT NULL GROUP BY clean_name
             ) fd ON ri.clean_name = fd.clean_name
        JOIN (SELECT clean_name, MAX(r3.receipt_date) AS max_date FROM receipt_items ri3
              JOIN receipts r3 ON ri3.receipt_id = r3.id
              WHERE ri3.clean_name IS NOT NULL AND ri3.unit_price IS NOT NULL GROUP BY clean_name
             ) ld ON ri.clean_name = ld.clean_name
        WHERE ri.unit_price IS NOT NULL AND ri.clean_name IS NOT NULL
          AND ri.is_statiegeld = false AND ri.is_koopzegel = false AND r.parsed = true
        GROUP BY ri.clean_name, ri.category
        HAVING COUNT(DISTINCT ri.receipt_id) >= 3
        ORDER BY ABS(
          COALESCE(MIN(CASE WHEN r.receipt_date = ld.max_date THEN ri.unit_price END), 0) -
          COALESCE(MIN(CASE WHEN r.receipt_date = fd.min_date THEN ri.unit_price END), 0)
        ) DESC NULLS LAST LIMIT 6
      `,
    ])
  ...
  return {
    week: weekData[0] ? toPlain([weekData[0]])[0] : null,
    month: monthData[0] ? toPlain([monthData[0]])[0] : null,
    weeklyChart: toPlain([...weeklyChart].reverse()),
    recentReceipts: toPlain(recentReceipts),
    categories: toPlain(categories),
    inflation: toPlain(inflation),
...
async function getMealPlan() {
  try {
    ...
    return rows[0] ? toPlain([rows[0]])[0] : null
  } catch { return null }
}
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-[310px_1fr] gap-4">
        ...
        <Suspense fallback={<CardSkeleton />}>
          <SpendChartClient data={data.weeklyChart} weekBudget={weekBudget} />
        </Suspense>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
        <InsightsPanel week={data.week as any} forecast={{ monthSpend, target: weekBudget * 4.33 }} />
