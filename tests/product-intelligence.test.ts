...
test('recommendDealsForProducts downgrades pantry-covered deals', () => {
  const recommendations = recommendDealsForProducts(
    [
      { name: 'AH Halfvolle Melk', discount: '2 voor €3', category: 'Zuivel & Eieren' },
    ],
  ...
  assert.equal(recommendations.length, 1)
  assert.equal(recommendations[0]?.pantry_match, true)
  assert.equal(recommendations[0]?.recommendation, 'good_if_needed')
})
